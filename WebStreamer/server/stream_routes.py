...
async def dl_route_handler(request: web.Request):
    try:
        ...
        return await media_streamer(request, path, filename)
    except InvalidHash as e:
        raise web.HTTPForbidden(text=e.message)
    except FIleNotFound as e:
        raise web.HTTPNotFound(text=e.message)
...
async def media_streamer(request: web.Request, path: str, filename: str):
    ...
    mime_type = file_id.mime_type or mimetypes.guess_type(filename)[0] or "application/octet-stream"
